...
// Helper function to safely parse dates that can be either string or Date objects
const safeParseDate = (date: string | Date): Date => {
  // If it's already a Date object (from Prisma), just return it
...
export function rangeText(view: TCalendarView, date: Date): string {
  ...
  return `${format(start, FORMAT_STRING, { locale: fr })} - ${format(
    end,
    FORMAT_STRING,
    { locale: fr }
  )}`;
...
export function getEventsCount(
  ...
  return events.filter((event) => {
    ...
    return compareFn(eventStartDate, date);
  }).length;
}
...
export function groupEvents(dayEvents: IEvent[]): IEvent[][] {
  const sortedEvents = dayEvents.sort(
    (a, b) =>
      safeParseDate(a.startDate).getTime() -
      safeParseDate(b.startDate).getTime()
  );
  const groups: IEvent[][] = [];

  for (const event of sortedEvents) {
    const eventStart = safeParseDate(event.startDate);
    let placed = false;

    for (const group of groups) {
      const lastEventInGroup = group[group.length - 1];
      const lastEventEnd = safeParseDate(lastEventInGroup.endDate);
...
export function calculateMonthEventPositions(
  ...
  const sortedEvents = [
    ...multiDayEvents.sort((a, b) => {
      const aDuration = differenceInDays(
        safeParseDate(a.endDate),
        safeParseDate(a.startDate)
      );
      const bDuration = differenceInDays(
        safeParseDate(b.endDate),
        safeParseDate(b.startDate)
      );
      return (
        bDuration - aDuration ||
        safeParseDate(a.startDate).getTime() -
  ...
  sortedEvents.forEach((event) => {
    const eventStart = safeParseDate(event.startDate);
    const eventEnd = safeParseDate(event.endDate);
...
export const getEventsForDay = (
  ...
  return events
    .filter((event) => {
      ...
      const startOfDayForEventEnd = startOfDay(safeParseDate(event.endDate));
      if (isWeek) {
        return (
    ...
    .map((event) => {
      const eventStart = startOfDay(safeParseDate(event.startDate));
      const eventEnd = startOfDay(safeParseDate(event.endDate));
...
// Get visual variant based on appointment status
export const getAppointmentVariant = (
  status?: "NOT_YET_ATTENDED" | "ATTENDED" | "ABSENT" | "CANCELLED",
  paid?: boolean
): TEventColor => {
  // Priority: Status takes precedence over payment
  if (status === "ATTENDED") return "green"; // attended = always green
  if (status === "CANCELLED") return "orange"; // cancelled = orange
  if (status === "ABSENT") return "red"; // absent = red

  // NOT_YET_ATTENDED or default - payment status matters
  return paid ? "blue" : "yellow"; // upcoming paid = blue, upcoming unpaid = yellow
};

// Update wrappers to accept paid flag but keep backward compatibility
export const getAppointmentClasses = (
  status?: "NOT_YET_ATTENDED" | "ATTENDED" | "ABSENT" | "CANCELLED",
  paid?: boolean
): string => {
  return getColorClass(getAppointmentVariant(status, paid));
};

export const getAppointmentBgClasses = (
  status?: "NOT_YET_ATTENDED" | "ATTENDED" | "ABSENT" | "CANCELLED",
  paid?: boolean
): string => {
  return getBgColor(getAppointmentVariant(status, paid));
};
